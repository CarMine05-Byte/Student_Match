...
def notifica(request, id_gruppo=None):
    user = request.session.get("user")
    ruolo = request.session.get("ruolo")
    ...
    if id_gruppo is not None:
        ...
        if not puo_leggere_notifiche_gruppo(utente, ruolo, gruppo):
            return home(request, {
                "error": "Non sei autorizzato a leggere le notifiche di questo gruppo."
            })
        ...
        notifiche = notifica_ric_gruppo(utente, gruppo)
        gruppi_gestiti = Gestione.objects.none()

    context = {
        "notifiche": notifiche,
        "gruppi_gestiti": gruppi_gestiti,
        "gruppo": gruppo,
        "puo_inviare": puo_inviare,
    }
        ...
        if azione == "invia":
            ...
            if gruppo:
                ...
                if not id_gruppo:
                    ...
                    gruppo_scelto = Gruppo.objects.filter(id_gruppo=id_gruppo).first()
                    context.update(notifica_inv(admin, gruppo_scelto, testo))
        elif azione == "letta":
            if gruppo:
                lette = notifica_lette_gruppo(utente, gruppo)
                context["success"] = f"{lette} notifiche del gruppo segnate come lette."
            else:
                lette = notifica_lette(utente)
                context["success"] = f"{lette} notifiche segnate come lette."
        elif azione == "nascondi":
            notifica_id = request.POST.get("notifica_id")
            aggiornata = notifica_letta_singola(utente, notifica_id, gruppo)
            if aggiornata:
                context["success"] = "Notifica segnata come letta."
            else:
                context["error"] = "Notifica non trovata."

        context["notifiche"] = (
            notifica_ric_gruppo(utente, gruppo)
            if gruppo else notifica_ric(utente)
        )

    return render(request, "notifiche.html", context)


def puo_leggere_notifiche_gruppo(utente, ruolo, gruppo):
    if ruolo == "studente":
        studente = Studente.objects.filter(studente=utente).first()
        return (
            Partecipazione.objects.filter(
    ...
    if ruolo == "tutor":
        tutor = Tutor.objects.filter(tutor=utente).first()
        return (
            Supporto.objects.filter(tutor=tutor, id_gruppo=gruppo).exists()
            if tutor else False
        )
...
def aggiorna_gruppo(request, gruppo, context):
...
def modifica_gruppo(request, id_gruppo):
    user = request.session.get("user")
    ruolo = request.session.get("ruolo")
    ...
    if ruolo != "admin":
        return home(request, {"error": "Solo gli admin possono modificare i gruppi."})
    ...
    if not gestisce_gruppo:
        return home(request, {
            "error": "Non sei autorizzato a modificare questo gruppo."
        })
    ...
    if request.method == "POST":
        aggiorna_gruppo(request, gruppo, context)

    return render(request, "modifica_gruppo.html", context)
...
def notifica_ric_gruppo(utente, gruppo):
    admin_gruppo = Gestione.objects.filter(
        id_gruppo=gruppo
    ).values_list("admin_id", flat=True)

    return Invio.objects.filter(
        destinatario=utente,
        mittente_id__in=admin_gruppo,
        letta=False,
    ).select_related(
        "mittente",
        "mittente__admin",
...
def notifica_lette_gruppo(utente, gruppo):
    admin_gruppo = Gestione.objects.filter(
        id_gruppo=gruppo
    ).values_list("admin_id", flat=True)
...
def notifica_letta_singola(utente, notifica_id, gruppo=None):
    notifiche = Invio.objects.filter(
        id=notifica_id,
        destinatario=utente,
        letta=False,
    )
    ...
    if gruppo:
        ...
        notifiche = notifiche.filter(mittente_id__in=admin_gruppo)

    return notifiche.update(letta=True)
